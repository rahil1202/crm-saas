...
export function OutreachSettingsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!disposed) setSettings(response.outreachAgent);
      } catch (caughtError) {
  ...
  const runNow = async () => {
    try {
      await apiRequest("/outreach/run-now", { method: "POST", body: JSON.stringify({}) });
    } catch (caughtError) {
      ...
      <Card className="border-border/70">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button type="button" variant="outline" onClick={runNow}>Run now</Button>
          </div>
        </CardContent>
      </Card>
    </div>
